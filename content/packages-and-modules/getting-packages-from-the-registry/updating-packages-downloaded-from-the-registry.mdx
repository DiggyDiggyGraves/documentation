import {
  Note,
  YouTube
} from '~/mdx'Updating local and global packages you downloaded from the registry helps keep your code and tools stable, usable, and secure.
...
1. In your project root directory, run the [`update` command][npm-update]:

   ```
   npm update
   ```

2. To test the update, run the [`outdated` command][npm-outdated]. There should not be any output.
